...
- さくらのVPSではクラウドシェル上でtailscale入れてしまう。ssh設定のため。
```
...
- SSL常時リダイレクト＋接続をクライアントのtailnetのIPからのみ許可
```
...
```
(client browser) https://nextcloud.tailXXXXX.ts.net ## 接続成功する。It works!ページが表示される
(client browser) https://nextcloud.tailXXXXX.ts.net/hoge  ## 接続成功するが、Not Found The requested URL was not found on this server.とだけ表示され、ほかの情報が出ないのを確認する
(client browser) ${サーバーのIPv4アドレス or IPv6アドレス}   ## 接続がタイムアウトしページが表示されないことを確認する
(client browser) ${VPS業者が割り当てたデフォルトドメイン}   ## 接続がタイムアウトしページが表示されないことを確認する
```
